...
generate_sample_data <- function(n_days = 180, n_products = 20) {
  ...
  revenue_trend <- data.frame(
    ...
    revenue = pmax(
      1000,
      2000 + 500 * sin(seq_along(dates) * 0.3) + rnorm(length(dates), 0, 200)
    ),
    orders = pmax(
      10,
      50 + 20 * sin(seq_along(dates) * 0.2) + rpois(length(dates), 10)
    ),
    users = pmax(
      5,
      25 + 15 * sin(seq_along(dates) * 0.25) + rpois(length(dates), 5)
    )
  )
...
filter_data <- function(
  ...
  filtered_revenue <- data$revenue_trend[
    data$revenue_trend$date >= as.character(start_date),
  ]
  ...
  if (length(selected_categories) > 0) {
    ...
    if (length(selected_labels) > 0) {
      filtered_products <- filtered_products[
        filtered_products$category %in% selected_labels,
      ]
    }
  }
  ...
  if (nrow(filtered_products) > 0) {
    filtered_category_performance <- aggregate(
      cbind(
        sales = filtered_products$sales,
        revenue = filtered_products$revenue
      ),
...
calculate_metrics <- function(current_data, previous_data = NULL) {
  if (is.null(previous_data)) {
    ...
    previous_data$revenue_trend$revenue <- current_data$revenue_trend$revenue *
      runif(1, 0.8, 1.1)
    previous_data$products$sales <- current_data$products$sales *
      runif(nrow(current_data$products), 0.8, 1.1)
  }
  ...
  current_conversion <- if (current_users > 0) {
    (current_orders / current_users) * 100
  } else {
    0
  }
  ...
  previous_conversion <- if (previous_users > 0) {
    (previous_orders / previous_users) * 100
  } else {
    0
  }
